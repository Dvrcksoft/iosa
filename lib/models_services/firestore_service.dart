...
import 'package:signalbyt/models/app_controls_public.dart';
...
/* ---------------------------- NOTE NEWS SERVICE --------------------------- */
  ...
  static Stream<List<SignalAggrOpen>> streamSignalsAggrOpen() {
    var ref =
        FirebaseFirestore.instance.collection('signalsAggrOpen').where('isEnabled', isEqualTo: true).orderBy('sort', descending: false).snapshots();
  ...
  static Future<List<Signal>> getSignals({String type = 'crypto'}) async {
    try {
      ...
      var ref =
          await FirebaseFirestore.instance.collection(colName).where('isClosed', isEqualTo: true).orderBy('timestampClosed', descending: true).get();
