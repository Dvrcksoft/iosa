    ...
    return Scaffold(
      body: ListView(
        children: [
          ...
          _buildHeading(
              title: 'Announcements',
              onTap: () => Get.to(() => AnnoucementsPage(announcements: announcements), fullscreenDialog: true),
              showViewAll: announcements.length > 2),
          Column(children: [for (var post in announcementsFirst5) ZAnnoucementCard(announcement: post)]),
          SizedBox(height: 16),
          /* ---------------------------------- NEWS ---------------------------------- */
          if (news.length > 0)
            _buildHeading(title: 'News', onTap: () => Get.to(() => NewsPage(news: news), fullscreenDialog: true), showViewAll: true),
